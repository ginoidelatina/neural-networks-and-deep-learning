...
print("sigmoid([0, 2]) = " + str(sigmoid(np.array([0,2]))))x = np.array([0.5, 0, 2.0])
output = sigmoid(x)
print(output)
...
def initialize_with_zeros(dim):
    ...
    dim - tamanho do vetor w que queremos (ou número de parâmetros neste caso)
...
def propagate(w, b, X, Y):
    ...
    Implemente a função de custo e seu gradiente para a 
    propagação explicada acima
    ...
    # FORWARD PROPAGATION
    m = X.shape[1]  # Qtd. de exemplos de treino.

    # compute activation
    A = sigmoid(np.dot(w.T, X) + b) # 1)
    
    # compute cost
    cost = -1/m*(np.dot(Y, np.log(A).T) + np.dot((1-Y), np.log(1-A).T))  # 2)
     
    # BACKWARD PROPAGATION (TO FIND GRAD)
    dw = 1/m*(np.dot(X, (A - Y).T)) # 3) 
    db = 1/m*(np.sum(A - Y)) # 4)
...
############################################# Exercise 8 - Further analysis (optional/ungraded exercise)
